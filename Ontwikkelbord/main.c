/*
* P1UnoBridge-PIC18F425
* JMR
*
* Compile: C:\HTSOFT\PIC18\bin\picc18.exe -C -E"Error.Compile.log" "main.c" -O"main.obj" -Zg9 -O -ASMLIST -18F452
* Link: C:\HTSOFT\PIC18\bin\picc18.exe -E"Error.Link.log" "main.obj" -M"Projectmain.map" -O"Projectmain.cof" -O"Projectmain.hex" -18F452
*
...
void main(void) {
    ...
    DisplayInit(); 
    DisplayCommand(LCD_TWOLINE);
    DisplayCommand(LCD_STDENTRY);
    DisplayCommand(LCD_ON);  
    DisplayCommand(LCD_CLS); 
    DisplaySend("Typmaschien v0.0.2B ", 20); //regel 1
    DisplaySend("info@regeling.com   ", 20); //regel 3
    DisplaySend("J.M. Regeling       ", 20); //regel 2
    DisplaySend("http://regeling.com ", 20); //regel 4
    ...
    while(1) {
        if (key != KEYBOARD_UNDEFINED_STATE) { //0xFF) {
            ...
            SerialSendByte(keyChar);

            //lcd_data(keyChar);
            DisplaySend(&keyChar, 1);
        ...
        if (RXshowData == 1) {
            RXshowData = 0;
            DisplayCommand(LCD_CLS); 
            DisplaySend("Typmaschien v0.0.2  ", 20); //regel 1
            DisplaySend(&RXbuffer, 20); //regel 3
            DisplaySend("Data received:      ", 20); //regel 2
            DisplaySend("                    ", 20); //regel 4
        }
    }
}
...
void interrupt interrupt_handler(void) {
    ...
    if (INT0IF) { // INT0 = Keyboard interrupt
        //Toggle(RE0);
        SerialSendStringInterrupt("intK");
        ...
        SerialSendStringInterrupt("int1");
        SerialSendByteInterrupt(RXptr + 0x30);
        
        SerialSendStringInterrupt(&RXbuffer);
        SerialSendStringInterrupt("\r\n");
        
    }
    else if (INT2IF) { //INT2 (switch 2)
        //Toggle(RE2);
        //send_byte_UART_int('2');
        SerialSendStringInterrupt("int2");
    ...
    //}
        ...
        RXbuffer[RXptr] = SerialReceivedByte();
        RXptr++;
        if (RXptr > 31) {
            RXptr = 0;
            RXshowData = 1;
